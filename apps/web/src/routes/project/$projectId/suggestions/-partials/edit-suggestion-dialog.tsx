...
import { SUGGESTION_CATEGORIES, type SuggestionCategory } from '@app/types';
...
export interface EditSuggestionDialogProps {
	...
	defaultValues: { description: string; category: SuggestionCategory };
	onSubmit: (values: { description: string; category: SuggestionCategory }) => void;
	isUpdating: boolean;
}

export const EditSuggestionDialog = ({ open, onOpenChange, defaultValues, onSubmit, isUpdating }: EditSuggestionDialogProps) => {
	const form = useForm({
		defaultValues,
		validators: { onChange: EditSuggestionSchema, onSubmit: EditSuggestionSchema },
					...
					<FieldGroup className="gap-4">
						...
						<div className="flex gap-2 justify-end pt-2">
							...
							<Button type="submit" disabled={isUpdating}>
								{isUpdating ? <HugeiconsIcon icon={Loading03Icon} className="size-4 animate-spin" /> : 'Save'}
							</Button>
						</div>
					</FieldGroup>
				</form>
